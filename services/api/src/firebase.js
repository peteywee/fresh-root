...
export function initFirebase(projectId, googleCredsJson) {
    if (getApps().length === 0) {
        if (googleCredsJson) {
            try {
                ...
                console.warn("[firebase] Invalid GOOGLE_APPLICATION_CREDENTIALS_JSON, falling back to default credentials");
