...
app.post("/session", async (req, res) => {
    try {
        ...
        // Secure cookie flags
        res.setHeader("Set-Cookie", `session=${encodeURIComponent(sessionCookie)}; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=${Math.floor(expiresIn / 1000)}`);
...
app.post("/orgs/:orgId/schedules", requireSession, require2FAForManagers, sameOrgGuard((req) => req.params.orgId), requireManager, async (req, res) => {
    try {
...
app.get("/orgs/:orgId/schedules", requireSession, sameOrgGuard((req) => req.params.orgId), async (req, res) => {
    try {
        const { orgId } = req.params;
        const cached = await cache.get(`schedules:${orgId}`);
        if (cached)
