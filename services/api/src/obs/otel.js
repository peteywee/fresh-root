...
function loadOTLPTraceExporter() {
    ...
    throw new Error("No OTLP trace exporter package found. Install one of: @opentelemetry/exporter-trace-otlp-http, @opentelemetry/exporter-trace-otlp-grpc, or @opentelemetry/exporter-trace-otlp-proto.");
}
/**
...
export function initOTel(config) {
    ...
    try {
        ...
        try {
            ...
            const attrs = [];
            if (serviceVersion)
                attrs.push(`service.version=${serviceVersion}`);
            if (attrs.length) {
                ...
                getNodeAutoInstrumentations({
                    ...
                    "@opentelemetry/instrumentation-http": {
                        ...
                        requestHook: (span, request) => {
                            const userAgent = "headers" in request && request.headers
 ...
 */
export async function flushTraces(_timeout = 5000) {
    if (sdk) {
        await sdk.shutdown();
