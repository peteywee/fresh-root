...
```
LEVEL 0: Constitution (cannot override)
  └─ Anti-vaporware, truth & evidence, security supremacy

LEVEL 1: Canonical 12-Document System
  └─ Location: .github/governance/ (after execution)
  └─ Only source of truth for project governance
  └─ Supercedence: 01 > 02 > 03 > ... > 12

LEVEL 2: Operational References
  └─ agents/crewops.md (kept)
  └─ docs/guides/crewops/* (kept)
  └─ docs/standards/CODING_RULES_AND_PATTERNS.md (kept)

LEVEL 3: TRASH (31 files)
  └─ .github/instructions/* (all deleted)
  └─ .github/ governance files (deleted)
  └─ docs/governance.md and reconciled-rulebook.md (deleted)
```
...
2. **NO ARCHIVES** - Old files are TRASH, not archived
3. **HIERARCHY RESPECTED** - Old files conform to canonical
4. **LOGIC VERIFIED** - Check all gates before proceeding
5. **PARALLEL WHERE POSSIBLE** - Don't serialize unnecessarily
...
4. Open A3 gate (if logic validates)
5. Start TRACK B & C in parallel
6. Execute TRACK D after B3 & C3 complete
...
- Hierarchy explicitly documented
...
- **Logic Completeness**: 99% (all 48 files accounted for)
- **Hierarchy Validity**: 99% (circular deps verified)
- **Task Clarity**: 99% (every step explicit)
- **Parallel Optimization**: 95% (gates required for ordering)
- **Execution Safety**: 99% (validation at every gate)
...
**READ**: `EXECUTION_MASTER_PLAN.md` (5 minutes)

**THEN**: Start TRACK A1

**SUCCESS**: Guaranteed when you follow the plan.
