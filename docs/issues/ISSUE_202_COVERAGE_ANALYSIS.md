...
2. **tenant-isolation.test.ts** (~70 lines)
   - ✅ Cross-org read prevention
   - ✅ Basic tenant isolation
   - ✅ Schedule isolation between orgs

3. **roles-schedules.test.ts** (~130 lines)
   - ✅ RBAC for schedules
   - ✅ Role hierarchy enforcement
   - ⚠️ Limited to schedules only

4. **helpers.ts** (~90 lines)
   - ✅ Test environment setup
   - ✅ Context builders
   - ✅ Seed functions

5. **rules-smoke.spec.mts** (~160 lines)
   - ✅ Basic smoke tests
...
8. `/orgs/{orgId}/join_tokens/{tokenId}`
9. `/organizations/{orgId}/messages/{messageId}`
10. `/organizations/{orgId}/receipts/{receiptId}`
11. `/organizations/{orgId}/schedules/{scheduleId}`
...
16. `/schedules/{orgId}/schedules/{scheduleId}`
17. `/shifts/{orgId}/shifts/{shiftId}`
18. `/attendance_records/{orgId}/records/{recordId}`
19. `/join_tokens/{orgId}/join_tokens/{tokenId}`
