...
title: "PostgreSQL Migration Strategy: Firebase Exit Plan"
description: "Comprehensive strategy for migrating from Firebase to PostgreSQL."
keywords:
  - migration
  - postgresql
  - firebase
...
1. **Order of migration (by risk)** 2\. `GET /api/ops/*` (internal only) 3\. `GET /api/positions`
   (simple CRUD) 4\. `GET /api/venues` (simple CRUD) 5\. `GET /api/schedules` (medium complexity)
   6\. `GET /api/shifts` (high volume) 7\. Auth-related reads (highest risk)
