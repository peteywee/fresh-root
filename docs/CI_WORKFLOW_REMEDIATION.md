...
1. Added pnpm overrides in `package.json`:
   ...
   ```
2. Replaced vulnerable `xlsx` with `exceljs` in `apps/web/src/lib/imports/_template.import.ts`
3. Ran `pnpm store prune && pnpm install` to force override application
