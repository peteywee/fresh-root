# Project Bible – v15 Migration Plan (Technical)
...
## 0. Global Constraints & Definitions

### 0.1 Layer Model (Hard Constraint)

All changes must respect the layering model:

- **LAYER_00_DOMAIN_KERNEL** – `packages/types/src/**`
- **LAYER_01_INFRASTRUCTURE** – `services/api/src/**`, `apps/web/src/lib/env*.ts`, `apps/web/src/lib/firebase.server.ts`, rules
- **LAYER_02_APPLICATION_LIBS** – `apps/web/src/lib/**` (excluding env/firebase server)
- **LAYER_03_API_EDGE** – `apps/web/app/api/**`, `apps/web/app/api/_shared/**`, `apps/web/middleware.ts`, `services/api/src/index.ts`
- **LAYER_04_UI_UX** – `apps/web/app/**`, `apps/web/components/**`
...
### 0.2 Tenant & Attendance Model (Canonical v15)

Canonical v15 domain chain:
...
Network
  └── Org
        └── Venue
              ├── Staff/User
              ├── Shift
              ├── Role    (position on a shift: Server, Cook, MOD, etc.)
              └── Attendance (user + shift + role + status/timestamps)
```
...
## 1. Phase 1 – Inventory & Patterns (What We Have)

### 1.1 Code Inventory Generation

**Goal:** Have a machine-readable dump of the current file structure (apps, packages, services).

**Command (from repo root):**

```bash
pnpm exec tree apps packages services -I "node_modules|.next|dist" -f > docs/migration/v15/PHASE1_CODE_INVENTORY.txt
```
...
**Goal:** Single table mapping domain entities to Bible, types, Firestore, and layers.

**Document:** `docs/migration/v15/01_ENTITY_INVENTORY.md` (already scaffolded with Role row).
...
### 1.3 Pattern Map (Layering + Domain + Tagging)

**Document:** `docs/migration/v15/PHASE1_PATTERN_MAP.md` (already updated with Role in chain).
