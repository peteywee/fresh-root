...
(async () => {
  ...
  try {
    ...
    const found = await page.waitForSelector(preferredSelector, { timeout: 10000 }).catch(() => null);

  if (!found) {
      // Fallback: list all buttons for debugging
      const buttons = await page.$$eval('button', nodes => nodes.map(n => ({text: n.innerText, class: n.className})).slice(0,50));
      console.error('[main] Google sign-in button not found; page buttons:', JSON.stringify(buttons, null, 2));
  } else {
      ...
      if (popup) {
        ...
        console.error('[main] No popup and no navigation detected after click — popup likely blocked or click did not invoke provider.');
      }
    }
