...
class SystemMonitor {
  ...
  async render() {
    ...
    this.viz.drawMetricBox('Memory', `${metrics.memory.used}MB / ${metrics.memory.total}MB`, '\x1b[34m');
    ...
    if (topProcs.length > 0) {
      ...
      console.log('\x1b[90m' + 'Process'.padEnd(30) + 'CPU%'.padStart(8) + 'MEM%'.padStart(8) + '\x1b[0m');
      ...
      topProcs.forEach(proc => {
        ...
        console.log(
          ...
          cpuColor + proc.cpu.toFixed(1).padStart(8) + '\x1b[0m' +
          '\x1b[36m' + proc.mem.toFixed(1).padStart(8) + '\x1b[0m'
        );
      });
    }
