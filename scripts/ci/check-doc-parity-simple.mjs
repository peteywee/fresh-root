...
async function main() {
  ...
  for (const s of schemaExports) {
    ...
    const ok = await ensureExists(path.join(root, doc));
    if (!ok)
      problems.push({ kind: 'schema-doc-missing', file: s.file, schema: s.name, need: doc });
  }
  ...
  for (const df of allDocs) {
    ...
    if (!/TEST SPEC/i.test(t)) problems.push({ kind: 'doc-missing-test-link', file: path.relative(root, df) });
  }
