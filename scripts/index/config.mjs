...
export const CATEGORIES = [
  ...
  { name: "Tests", match: p => p.startsWith("tests/") || /(^|\/)__tests__\//.test(p) },
  {
    ...
    match: p =>
      p.startsWith(".github/") ||
      /(^|\/)(tsconfig.*|eslint.*|vitest.*|jest.*|turbo\.json|pnpm-.*\.yaml|pnpm-workspace\.yaml|firebase.*\.json|firestore.*\.json|tailwind.*\.cjs|tailwind.*\.ts|postcss.*|cspell\.json|\.prettier.*|\.markdownlint\.json|\.mcp\.json|eslint\.config\.mjs|vitest\.config\.ts|jest\.config\.ts)$/i.test(p),
  },
