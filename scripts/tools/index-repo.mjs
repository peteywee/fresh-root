...
const PATTERNS = {
  ...
  firestore:
    /\bfirestore\b|admin\.firestore|firebase\.firestore|getFirestore\(|collection\(|doc\(/gi,
...
function groupByType(file) {
  ...
  if (ext === ".ts" || ext === ".tsx" || ext === ".js" || ext === ".jsx" || ext === ".mts")
    return "code";
...
async function index() {
  ...
  for (const k of Object.keys(PATTERNS)) {
    ...
    summary.topFiles[k] = sorted
      .slice(0, 10)
      .filter((e) => (e.stats[k] || 0) > 0)
      .map((e) => ({ path: e.path, count: e.stats[k] }));
  }
  ...
  await fs.writeFile(
    ...
    JSON.stringify({ result, summary, groups: groupMap }, null, 2),
    "utf8",
  );
  ...
  for (const [group, files] of Object.entries(groupMap)) {
    ...
    files.slice(0, 200).forEach((f) => {
      md.push(
        `- ${f.path} — size=${f.size} — todos=${f.stats.todo || 0} any=${f.stats.anyCast || 0} firestore=${f.stats.firestore || 0}`,
      );
    });
  }
