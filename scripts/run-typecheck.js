...
function findTsconfigs(dir) {
  ...
  while (stack.length) {
    ...
    if (entries.includes('tsconfig.json')) {
      ...
      try {
        const cfg = require('fs').readFileSync(require('path').resolve(cur, 'tsconfig.json'), 'utf8').trim();
        if (!cfg) {
...
try {
  ...
  console.error('Could not resolve tsc. Make sure TypeScript is installed in the workspace devDependencies.');
  process.exit(2);
}
