...
class InteractiveOrchestrator {
    ...
    async saveState() {
        await fs.writeFile(path.join(this.repoRoot, STATE_FILE), JSON.stringify(this.state, null, 2), "utf-8");
    }
    prompt(question) {
    ...
    showPhases() {
        ...
        PHASES.forEach((phase, idx) => {
            ...
            console.log(`   ⏱️  ${phase.duration} | Risk: ${phase.riskLevel.toUpperCase()} | Status: ${isCompleted ? "DONE" : "PENDING"}`);
            console.log();
        });
    }
    showMenu() {
    ...
    async rollback() {
        const confirm = await this.prompt("⚠️  Are you sure you want to rollback ALL changes? (yes/no): ");
        if (confirm === "yes") {
            console.log("\n🔄 Rolling back...");
