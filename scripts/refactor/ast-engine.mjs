...
async function createBackup(filePath, content) {
    ...
    await fs.writeFile(backupPath, JSON.stringify({ filePath, content, timestamp: new Date().toISOString() }), "utf-8");
    return backupPath;
}
...
function transformImports(content) {
    ...
    const reorganized = importLines.reduce((acc, cur) => {
        const isType = cur.line.includes("import type");
        if (isType)
            acc.typeImports.push(cur.line);
        else
            acc.valueImports.push(cur.line);
        return acc;
 ...
 */
function transformNaming(content) {
 ...
 */
function transformBarrels(content, filePath) {
 ...
 */
function validateLayout(filePath, content) {
    const issues = [];
...
async function refactorFile(filePath, options = {}) {
    const changes = [];
    const diffs = [];
    try {
...
// --- MANIFEST GENERATION ---
async function generateManifest(allChanges) {
    const byType = allChanges.reduce((acc, change) => {
...
// --- ROLLBACK FUNCTION ---
async function rollback(manifestPath = MANIFEST_FILE) {
    try {
...
// --- MAIN EXPORT ---
export { refactorFile, generateManifest, rollback, generateUnifiedDiff, hashContent, validateLayout, };
