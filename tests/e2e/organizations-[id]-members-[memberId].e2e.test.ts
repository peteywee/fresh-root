...
describe("organizations-[id]-members-[memberId] API E2E Tests", () => {
  ...
  describe("GET /api/organizations/[id]/members/[memberId]", () => {
    ...
    it("should return 200 for valid request", async () => {
      const response = await fetch(`${BASE_URL}/api/organizations/[id]/members/[memberId]`, { headers: authHeaders });
      expect(response.status).toBe(401);
    });

    
    it("should return 401 without authentication", async () => {
      const response = await fetch(`${BASE_URL}/api/organizations/[id]/members/[memberId]`);
      expect(response.status).toBe(401);
    });
