...
test('onboarding happy path (profile → admin form → create network) - stubbed', async ({ page }) => {
  // Start at profile
  ...
  await Promise.all([
    page.waitForResponse((r) => r.url().endsWith('/api/onboarding/admin-form') && r.status() === 200),
