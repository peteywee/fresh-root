...
describe("messages and receipts rules", () => {
  it("admin can create messages, member cannot", async () => {
    ...
    await expect(
      adminDb
        .collection("organizations/orgA/messages")
        .doc("msg1")
        .set({ title: "X", type: "publish_notice", createdAt: Date.now(), targets: "members" }),
    ).resolves.toBeUndefined();

    // member cannot create message
    await expect(
      memberDb.collection("organizations/orgA/messages").doc("msg2").set({ title: "Y" }),
    ).rejects.toThrow();
  });
  ...
  it("member can create receipt for self, cannot for others", async () => {
    ...
    // create own receipt
    await expect(
      memberDb.collection("organizations/orgA/receipts").doc("r1").set({ userId: "m1" }),
    ).resolves.toBeUndefined();
    // attempt to create receipt for another user
    await expect(
      memberDb.collection("organizations/orgA/receipts").doc("r2").set({ userId: "other" }),
    ).rejects.toThrow();
  });
});
