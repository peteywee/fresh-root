 ...
 */
export const PATCH = csrfProtection()(requireOrgMembership(requireRole("manager")(async (request, context) => {
    // Apply rate limiting
    const rateLimitResult = await rateLimit(request, RateLimits.api);
    if (rateLimitResult)
        return rateLimitResult;
    ...
    try {
        ...
        if (!validationResult.success) {
            return NextResponse.json({ error: "Invalid position data", details: validationResult.error.errors }, { status: 400 });
        }
 ...
 */
export const DELETE = csrfProtection()(requireOrgMembership(requireRole("admin")(async (request, context) => {
    // Apply rate limiting
    const rateLimitResult = await rateLimit(request, RateLimits.api);
    if (rateLimitResult)
        return rateLimitResult;
