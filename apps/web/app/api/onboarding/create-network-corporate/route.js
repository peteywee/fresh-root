...
export async function createNetworkCorporateHandler(req, injectedAdminDb = importedAdminDb) {
    if (!injectedAdminDb) {
    ...
    const declared = claims.selfDeclaredRole || claims.role;
    ...
    const { corporateName, brandName, industry, approxLocations, formToken } = body || {};
    if (!formToken)
        return NextResponse.json({ error: "missing_form_token" }, { status: 422 });
...
export const POST = withSecurity(async (req, _ctx) => {
