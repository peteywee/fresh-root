...
export function securityHeaders(response) {
    ...
    response.headers.set("Content-Security-Policy", "default-src 'self'; script-src 'self' 'unsafe-eval' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; font-src 'self' data:; connect-src 'self' https://*.googleapis.com https://*.firebaseio.com wss://*.firebaseio.com;");
...
export function securityStack(options) {
    const rateLimitMiddleware = rateLimit(options?.rateLimit?.maxRequests, options?.rateLimit?.windowMs);
    ...
    return async (req, handler) => {
        ...
        return corsMiddleware(req, (req1) => sizeLimitMiddleware(req1, (req2) => rateLimitMiddleware(req2, async (req3) => {
