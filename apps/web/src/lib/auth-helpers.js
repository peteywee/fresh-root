...
// Tags: P0, AUTH, CODE
import { GoogleAuthProvider, signInWithPopup, signInWithRedirect, isSignInWithEmailLink, sendSignInLinkToEmail, signInWithEmailLink, getRedirectResult, } from "firebase/auth";
...
export async function sendEmailLinkRobust(email) {
    try {
        if (!auth)
            throw new Error("Firebase auth is not initialized. Ensure NEXT_PUBLIC_FIREBASE_* env vars are set or enable emulators.");
