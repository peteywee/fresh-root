...
const db = adminDb;
export async function createNetworkWithOrgAndVenue(adminUid, payload, injectedDb) {
    ...
    if (!draft)
        throw new Error("admin_form_not_found");
    if (draft.userId !== adminUid)
        throw new Error("admin_form_ownership_mismatch");
    const networkRef = root.collection("networks").doc();
