...
function reducer(state: ErrorState, action: Action): ErrorState {
  switch (action.type) {
    case 'PUSH': return { messages: [...state.messages, action.message] };
    case 'POP': {
      const next = state.messages.slice();
      next.pop();
      return { messages: next };
    }
    case 'CLEAR': return { messages: [] };
  ...
  return (
    <ErrorCtx.Provider value={api}>
      ...
      {state.messages.length > 0 && (
        <div className="fixed bottom-4 right-4 max-w-sm rounded border bg-white shadow p-3 text-sm space-y-2 z-50">
          {state.messages.map((m, i) => (<div key={i}>{m}</div>))}
          <button className="rounded border px-2 py-1 text-xs" onClick={api.clearErrors}>Dismiss</button>
        </div>
      )}
    </ErrorCtx.Provider>
  );
