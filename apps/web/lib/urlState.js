...
import { useQueryState, parseAsString, parseAsInteger, parseAsStringEnum, parseAsIsoDateTime, parseAsBoolean, } from "nuqs";
/**
 * Hook: Calendar view state (day/week/month)
 * URL: ?view=week
 */
export function useScheduleView(defaultValue = "week") {
    return useQueryState("view", parseAsStringEnum(["day", "week", "month"]).withDefault(defaultValue));
}
/**
 * Hook: Selected date state
 * URL: ?date=2025-11-06
 */
export function useSelectedDate(defaultValue) {
